...
0. **Hermit session**: Invoke the `/claude-code-hermit:session-start` skill first to start or resume a hermit session. This ensures session discipline is active before any HA work begins.
...
- No endpoint: tell the operator to set `HOMEASSISTANT_LOCAL_URL` in `.env`.
